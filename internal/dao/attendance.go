...
// - 按 emp_id + 日期区间
// - 按日期倒序
func GetAttendanceByEmpID(empID string, startDate, endDate time.Time, page, pageSize int) ([]model.Attendance, int64, error) {
	dbConn := db.GetDB()
	...
	query := dbConn.Model(&model.Attendance{}).
		Where("emp_id = ?", empID).
...
func SearchAttendance(empID string, dptID uint, startDate, endDate time.Time, page, pageSize int) ([]model.AttendanceDetail, int64, error) {
	dbConn := db.GetDB()
	...
	var list []model.AttendanceDetail
	var total int64
...
func UpdateAttendance(id uint, updates map[string]interface{}) error {
	...
	res := dbConn.Model(&model.Attendance{}).
		Where("id = ?", id).
		Updates(updates)
	if res.Error != nil {
...
func DeleteAttendance(id uint) error {
	...
	res := dbConn.Delete(&model.Attendance{}, id)
	if res.Error != nil {
