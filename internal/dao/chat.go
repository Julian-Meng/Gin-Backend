...
func GetAIResponse(req model.AIRequest) (model.AIResponse, error) {
	...
	if apiBase == "" || modelID == "" {
		return model.AIResponse{}, fmt.Errorf("AI environment variables not properly set: AI_BASE_URL / AI_MODEL_ID required")
	}
	...
	if err != nil {
		return model.AIResponse{}, fmt.Errorf("failed to marshal request body: %v", err)
	}

	httpReq, err := http.NewRequest("POST", apiURL, bytes.NewBuffer(requestBody))
	if err != nil {
		return model.AIResponse{}, fmt.Errorf("failed to create request: %v", err)
	}
	...
	if err != nil {
		if provider == "ollama" {
			return model.AIResponse{}, fmt.Errorf("failed to call Ollama service: %v (请确认 Ollama 已启动，默认端口 11434)", err)
		}
		return model.AIResponse{}, fmt.Errorf("failed to call AI service: %v", err)
	}
	...
	if resp.StatusCode != http.StatusOK {
		...
		return model.AIResponse{}, fmt.Errorf("AI service returned status code: %d, body: %s", resp.StatusCode, bodyStr)
	}

	var rawResp chatCompletionResponse
	if err := json.NewDecoder(resp.Body).Decode(&rawResp); err != nil {
		return model.AIResponse{}, fmt.Errorf("failed to decode AI response: %v", err)
	}
	if len(rawResp.Choices) == 0 {
		return model.AIResponse{}, fmt.Errorf("AI response is empty")
	}

	content := sanitizeThink(rawResp.Choices[0].Message.Content)
	return model.AIResponse{Message: content}, nil
}
...
func CreateUserChatMessage(userEmpID, content string, onlineAdminEmpIDs []string) (model.ChatSession, model.ChatMessage, bool, error) {
	dbConn := db.GetDB()
	now := time.Now()
	hasOnlineAdmin := len(onlineAdminEmpIDs) > 0

	var outSession model.ChatSession
	var outMessage model.ChatMessage
	...
	err := dbConn.Transaction(func(tx *gorm.DB) error {
		...
		if session.AssignedAdminEmpID == nil {
			session.Status = model.ChatSessionStatusWaiting
		}
		...
		msg := model.ChatMessage{
			...
			SenderType:    model.ChatMessageSenderUser,
			SenderEmpID:   userEmpID,
			Content:       content,
			MsgType:       model.ChatMessageTypeText,
			DeliveryState: model.ChatDeliveryStateQueued,
		}
...
func CreateAdminChatMessage(sessionID uint, adminEmpID, content string) (model.ChatSession, model.ChatMessage, error) {
	dbConn := db.GetDB()
	now := time.Now()
	...
	err := dbConn.Transaction(func(tx *gorm.DB) error {
		...
		session.Status = model.ChatSessionStatusOpen
		session.LastMessageAt = now
		session.Source = normalizeSessionSource(session.Source, model.ChatSessionSourceHuman)
		...
		msg := model.ChatMessage{
			SessionID:     session.ID,
			SenderType:    model.ChatMessageSenderAdmin,
...
func CreateAIFallbackMessage(sessionID uint, content, notice string) (model.ChatSession, model.ChatMessage, error) {
	dbConn := db.GetDB()
	now := time.Now()
	...
	err := dbConn.Transaction(func(tx *gorm.DB) error {
		...
		session.LastMessageAt = now
		session.Source = normalizeSessionSource(session.Source, model.ChatSessionSourceAIFallback)
		if err := tx.Save(&session).Error; err != nil {
			return err
		}
		...
		msg := model.ChatMessage{
			SessionID:     session.ID,
			SenderType:    model.ChatMessageSenderAI,
			Content:       content,
			MsgType:       model.ChatMessageTypeText,
			DeliveryState: model.ChatDeliveryStateQueued,
			AIFlag:        true,
...
func ClaimWaitingSessionsForAdmin(adminEmpID string, limit int) ([]model.ChatSession, error) {
	if limit <= 0 {
		limit = 20
	}

	dbConn := db.GetDB()
	claimed := make([]model.ChatSession, 0, limit)

	err := dbConn.Transaction(func(tx *gorm.DB) error {
		var candidates []model.ChatSession
		if err := tx.Where("status = ? AND assigned_admin_emp_id IS NULL", model.ChatSessionStatusWaiting).
		...
		for _, c := range candidates {
			res := tx.Model(&model.ChatSession{}).
				Where("id = ? AND status = ? AND assigned_admin_emp_id IS NULL", c.ID, model.ChatSessionStatusWaiting).
				Updates(map[string]interface{}{
					"assigned_admin_emp_id": adminEmpID,
					"status":                model.ChatSessionStatusOpen,
					"updated_at":            now,
				})
			if res.Error != nil {
				return res.Error
			...
			var claimedSession model.ChatSession
...
func ListSessionsForAdmin(adminEmpID string, limit int) ([]model.ChatSession, error) {
	if limit <= 0 {
		limit = 100
	}
...
func ListSessionsForUser(userEmpID string, limit int) ([]model.ChatSession, error) {
	if limit <= 0 {
		limit = 20
	}
	dbConn := db.GetDB()
...
func ListMessagesBySession(sessionID uint, limit int) ([]model.ChatMessage, error) {
	if limit <= 0 {
		limit = 200
	}
	dbConn := db.GetDB()
...
// GetSessionByID 查询会话。
func GetSessionByID(sessionID uint) (model.ChatSession, error) {
	dbConn := db.GetDB()
	var session model.ChatSession
...
func ClaimSessionByID(sessionID uint, adminEmpID string) (model.ChatSession, bool, error) {
	dbConn := db.GetDB()
...
func TouchMessagesAsDispatched(messageIDs []uint) error {
	...
	dbConn := db.GetDB()
	now := time.Now()
	return dbConn.Model(&model.ChatMessage{}).
		Where("id IN ? AND delivery_state = ?", messageIDs, model.ChatDeliveryStateQueued).
		Updates(map[string]interface{}{
			"delivery_state": model.ChatDeliveryStateDispatched,
			"delivered_at":   &now,
		}).Error
}

// GetQueuedMessagesBySession 查询会话中待派发消息。
func GetQueuedMessagesBySession(sessionID uint) ([]model.ChatMessage, error) {
	dbConn := db.GetDB()
	var messages []model.ChatMessage
	err := dbConn.Where("session_id = ? AND delivery_state = ?", sessionID, model.ChatDeliveryStateQueued).
		Order("id ASC").
		Find(&messages).Error
	return messages, err
}
...
func GetLatestAIFallbackMessage(sessionID uint) (model.ChatMessage, error) {
	dbConn := db.GetDB()
	var msg model.ChatMessage
	err := dbConn.Where("session_id = ? AND sender_type = ? AND ai_flag = ?", sessionID, model.ChatMessageSenderAI, true).
		Order("id DESC").
		First(&msg).Error
	return msg, err
}

func getOrCreateOpenSessionForUpdate(tx *gorm.DB, userEmpID string, now time.Time) (model.ChatSession, error) {
	var session model.ChatSession
	err := tx.Clauses(clause.Locking{Strength: "UPDATE"}).
		Where("user_emp_id = ? AND status <> ?", userEmpID, model.ChatSessionStatusClosed).
	...
	session = model.ChatSession{
		UserEmpID:     userEmpID,
		Status:        model.ChatSessionStatusWaiting,
		Source:        model.ChatSessionSourceHuman,
		LastMessageAt: now,
	}
...
func pickRandomLeastLoadedAdmin(tx *gorm.DB, onlineAdminEmpIDs []string) (string, error) {
	...
	if err := tx.Model(&model.ChatSession{}).
		...
		Where("assigned_admin_emp_id IN ? AND status <> ?", onlineAdminEmpIDs, model.ChatSessionStatusClosed).
...
func normalizeSessionSource(oldSource, incoming string) string {
	...
	if oldSource == model.ChatSessionSourceMixed {
		return oldSource
	}
	return model.ChatSessionSourceMixed
}
