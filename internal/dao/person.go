...
func FetchPersonsPaged(page, pageSize int, keyword string) ([]model.EmployeeInfo, int64, error) {
	dbConn := db.GetDB()

	offset := (page - 1) * pageSize
	var list []model.EmployeeInfo
	var total int64
	...
	countQuery := dbConn.Model(&model.Person{})
	if keyword != "" {
...
func FetchPersonByID(id uint) (*model.Person, error) {
	dbConn := db.GetDB()
...
func FetchPersonByEmpID(empID string) (*model.Person, error) {
	dbConn := db.GetDB()
...
func DeletePersonByEmpID(empID string) error {
	...
	return dbConn.Transaction(func(tx *gorm.DB) error {
		...
		res := tx.Delete(&model.Person{}, "emp_id = ?", empID)
		if res.Error != nil {
