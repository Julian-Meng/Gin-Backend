...
func CreatePersonnelChange(p *model.Personnel) error {
	dbConn := db.GetDB()
...
func FetchPersonnelPaged(limit, offset int) ([]model.Personnel, int64, error) {
	dbConn := db.GetDB()
	...
	// 统计总数
	if err := dbConn.Model(&model.Personnel{}).Count(&total).Error; err != nil {
...
func FetchPersonnelByEmpIDPaged(empID string, limit, offset int) ([]model.Personnel, int64, error) {
	dbConn := db.GetDB()
...
func GetPersonnelByID(id uint) (*model.Personnel, error) {
	dbConn := db.GetDB()
