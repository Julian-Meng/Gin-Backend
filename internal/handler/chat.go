...
type chatPushPayload struct {
	...
	Session   *model.ChatSession `json:"session,omitempty"`
...
// @Accept json
// @Produce json
// @Param request body model.AIRequest true "AI 对话参数"
// @Success 200 {object} model.AIResponse
...
func ChatWithAI(c *gin.Context) {
	...
	aiResponse, err := dao.GetAIResponse(model.AIRequest{
		Message:   req.Message,
		SessionID: sessionID,
	})
	if err != nil {
...
// @Produce json
// @Security BearerAuth
// @Param request body model.UserSendChatRequest true "消息内容"
...
func UserSendChatMessage(c *gin.Context) {
	...
	var req model.UserSendChatRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		errorx.BadRequest(c, "请求格式错误", err)
		return
	}
...
// @Produce json
// @Security BearerAuth
// @Param request body model.AdminSendChatRequest true "消息内容"
...
func AdminSendChatMessage(c *gin.Context) {
	...
	var req model.AdminSendChatRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		errorx.BadRequest(c, "请求格式错误", err)
		return
	}
...
// @Produce json
// @Security BearerAuth
// @Param request body model.AdminClaimRequest false "认领参数（可空，默认 limit=20）"
...
func parseAdminClaimRequest(c *gin.Context) (model.AdminClaimRequest, error) {
	var req model.AdminClaimRequest
...
func dispatchQueuedMessagesForSession(sessionID uint) error {
	...
	for _, msg := range queued {
		...
		case model.ChatMessageSenderUser:
			if session.AssignedAdminEmpID != nil {
				sent = runtimeChatHub.pushToAdmin(*session.AssignedAdminEmpID, payload)
			}
		case model.ChatMessageSenderAdmin, model.ChatMessageSenderAI, model.ChatMessageSenderSystem:
...
func shouldUseAIFallback(session model.ChatSession) bool {
...
func tryGenerateAIFallback(session model.ChatSession, userContent string) (*model.ChatMessage, error) {
	...
	aiResp, err := dao.GetAIResponse(model.AIRequest{
...
func principalFromClaims(username, empID, role string, hasEmpID bool) (string, string, bool) {
	role = middleware.NormalizeRole(role)
